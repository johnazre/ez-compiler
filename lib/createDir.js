...
function createFilesAndDirs(dirs, files){
	...
	FS.readFile('package.json', 'utf8', (error, data) => {
		...
		if (options.indexOf('create') > -1 && options.indexOf('--stylus') > -1){
			scripts.stylus = 'stylus -w ./src/css -o ./dist/css';
...
module.exports = {
	default: function() {
		let dirs = ['./src', './src/css', './src/js', './src/assets', './dist', './dist/css', './dist/js', './dist/assets'],
			  files = ['./src/index.html',  './src/css/style.css', './src/js/app.js', './.gitignore', './ezconfig.json'];

		createFilesAndDirs(dirs, files);
	},

	angular1: function() {
		let dirs = ['./src', './src/controllers', './src/directives', './src/services', './src/css', './src/templates','./dist', './dist/css', './dist/controllers/', './dist/directives', './dist/services', './dist/templates'],
